...
def assess():
    ...
    revision_file_path = r"LABsE/SmallData27/bjv-bjvNT"
    reference_file_path = r"LABsE/SmallData27/aai-aai-small.txt"
    ...
    results = [
        {
            ...
            "revision": revision_lines[j],
            "reference": reference_lines[j],
        }
        for j in range(len(vrefs))
    ]

    save_sim_scores_to_file(sim_scores, "LABsE/references/sim_scores.txt")
    merge_files('LABsE/references/vref.txt', 'LABsE/references/sim_scores.txt', 'LABsE/references/merged_results.txt')
...
output = assess()
if output:
    save_results_to_file(output["results"], "./LABsE/Outputs/check.txt")
