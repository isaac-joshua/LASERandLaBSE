...
def get_sim_scores(
    ...
    rev_sents_output = [sent if sent.strip() else "[PAD]" for sent in rev_sents_output]
    ref_sents_output = [sent if sent.strip() else "[PAD]" for sent in ref_sents_output]
...
def assess():
    ...
    revision_file_path = "./data/aak-aak.txt" 
    reference_file_path = "./data/apu-apuNT.txt"
    ...
    for i, (rev_batch, ref_batch) in enumerate(tqdm(zip(rev_sents_batched, ref_sents_batched), total=len(rev_sents_batched))):
        ...
        rev_batch = [sent if sent.strip() else "[PAD]" for sent in rev_batch]
        ref_batch = [sent if sent.strip() else "[PAD]" for sent in ref_batch]
